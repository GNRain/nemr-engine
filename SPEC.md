...
| 1.127 | Revision | **F-10: "still the placeholder" is the absence of a real token, never the marker's presence; the marker lives inside `claudeAiOauth`; a leftover marker is cleared on first detection of a login.** Found by the Product Owner on the fresh VM (2026-09-08): `/login` succeeded and the required proof held — Claude Code's write landed on the host through the bind, same inode, real OAuth fields on the host file — yet `nemr status` kept saying NO LOGIN YET and `attach` kept its banner, because Claude Code rewrites the credential file as a JSON object and preserves unknown top-level keys, so the E-21 marker survived the login; the placeholder had also matched a real credential to the byte (648 before and after), which defeated a size check. **Built:** the placeholder is now `{"claudeAiOauth":{"_nemr_placeholder":…}}` — the marker inside the object a login replaces, with no token fields beside it (measured on the host, Claude Code 2.1.263: it answers *Not logged in · Please run /login* and leaves the file alone, where a blank token beside the marker reads as *OAuth session expired*); `is_placeholder` and the facts' `placeholder` are the marker with no `claudeAiOauth.accessToken`, so a real token beside any leftover marker is a login; the placeholder is not `blank`. **The clear:** `auth::scrub_placeholder_marker` removes a leftover marker (top level or nested) the first time a real token is seen beside it — from `status` and from the daemon's credential watch, i.e. at the write that lands the login — by textual removal of the marker member so every other byte stays as Claude Code wrote it, checked against the parsed value (re-serialisation only as the fallback), written in place so the bind-mounted inode and the mode are kept. **Proofs:** unit — a login beside the marker (the VM's shape) is Fresh, not the placeholder, and the placeholder is exactly NoLoginYet and not Blank; the clear yields byte for byte what Claude Code alone would have written, same inode, 0600, and never touches a placeholder, a blank clear, a clean login or garbage; both red with the old detection and a no-op clear. Host — the E-21 test now writes the VM's shape (a real-shaped object with the marker kept) next to the running session: `status` says present, the host file is the object alone, same inode, and the session sees the cleared file through the bind; then the dead-refresh clear lands as before. The human arm is rerun by the Product Owner on this build. | Claude Code |
| 1.128 | Revision | **F-14: the host credential is bound as a DIRECTORY over `/root/.claude`, not as a single file, so a login's write lands on the host.** The human arm on the fresh VM (E-21) showed `/login` succeed and the required proof still fail: the host file ended as the full real credential, yet host and session hashes differed. Measured (strace, 2026-09-08): Claude Code writes `.credentials.json` by writing `.credentials.json.tmp.<hex>` and `rename`-ing it over the target — a new inode each time. A single-file bind mounts one inode at `/root/.claude/.credentials.json`; a rename over that mount point is refused (EBUSY, measured), so the login's first (in-place) write reached the host while a later temp+rename write escaped onto a container-only inode, and the two diverged. **Built:** the engine writes the placeholder and binds a DEDICATED host directory — `~/.local/share/nemr/host-credential` (or the seam path's parent), holding only `.credentials.json` and the `projects`/`sessions` mount points — read-write over `/root/.claude`; the volume's M8 `projects`/`sessions` binds still land on the subpaths, ordered after (measured: `/root/.claude` on the host fs, `projects`/`sessions` on the volume). Every write, in place or by rename, now lands in that directory on the host, so host and session share the inode. The directory is NOT the host's own `~/.claude` (history, sessions, settings must not travel — D-02), and nemr never inherits the host's own login: a credential is never copied, because a second holder of one refresh token invalidates the first the moment either rotates (E-13). A machine that logged in before F-14 starts from the placeholder and runs `/login` once, like any device. **Migration (the spec is frozen at create):** `start` rewrites a pre-F14 single-file bind to the directory bind before the task exists (`ensure_credential_dir_bind`), reporting it once; a fresh project binds the directory from create. **F-12 becomes trivial:** a host-side rename reaches a running session with no re-bind, because the whole directory is bound. **Proofs:** unit — the migration rewrites the file bind to a directory bind in place, ordered before the subpath binds, idempotent, refusing a spec with no mounts; a pre-F14 host (a real `~/.claude/.credentials.json`) comes up as *no login yet* — the placeholder in the dedicated directory, never a copy; host — a fresh project binds the directory, a pre-F14 project is migrated at start (control: it starts as the single-file bind, via `NEMR_TEST_PRE_F14`), a host rename reaches a running session with no re-bind, and the two-write login (in place, then temp+rename) lands on the host with host and session holding the SAME inode and bytes — red under the old single-file bind (the rename refused, `NEMR_TEST_SKIP_F14_MIGRATION` keeping the file bind past start). The human arm now compares inodes as well as hashes, and reads the credential path from `nemr status` (F-14 moved it off `~/.claude`). The automated arm, which restarts the daemon under the seam, now waits for the old listener to go before the seamed daemon autostarts (F-9 makes a second daemon exit rather than displace one still answering) and asserts the seamed daemon took over — `nemr status` naming the seam path — before any credential assertion, so a daemon-restart failure can never read as a credential one; proven red by leaving a clean daemon up. | Claude Code |
| 1.129 | Revision | **F-11: create from the page.** The list's toolbar has a *create session* button; it opens one panel in the job area: a name (lowercase letters, digits and `-`, as the engine requires), an agent (Claude Code by default; the others the daemon's `Create` allows — Codex, marked unverified per F-84) and a quota picker of the engine's fixed presets (500MB, 2GB — the default — and 10GB; the Product Owner asked for 1GB and 5GB steps, which are not engine presets: the privileged helper `deploy/nemr-volume` hardcodes the three and every host would need it redeployed with sudo, so that is flagged in the PR, not done silently here). Submit runs a *create* job (`POST /api/sessions`, the daemon's `Create` RPC through `UiEngine::create`); on success the panel closes and the row appears, stopped, with its own start button; a refusal (a name in use, a size the engine does not allow) goes to the status line as F-3 defined. The options travel with every sessions reply (`create_options`), stated by the binary — the commercial half never links the engine (E-11) — and the browser acceptance holds the picker to exactly the sizes `nemr create` names when refused one (its control). **The acceptance's one stated exception is gone:** the session it pushes is created through the page's own route, and started through it. Proofs: the route test (a refused size and a name in use surface as the job's error, an empty name is refused before any job, the row lands stopped with the CLI's defaults); on the real page (headless Firefox) the panel's options and default, the row appearing with the panel closed, the outcome on the status line, and a refusal on the status line — red with the create submit neutered (the wait for the new row times out). | Claude Code |
| 1.130 | Revision | **F-12: remove from the page — from this machine; the cloud copy is never touched (E-22 opened for deleting it).** Every local row has a remove button that says which case it is: *remove from this machine* when a bundle exists in the cloud, *remove the only copy* when none does. The panel repeats the case: with a cloud copy, one click, no typing, and the row stays and reads remote afterwards; with no copy, the confirm button is disabled until the session's name is typed exactly. A running session is stopped first, the way stop-and-push stops it, and the panel says so. The job (`DELETE /api/sessions/{name}`, the daemon's `Delete` RPC through `UiEngine::delete`) says *removed … from this machine; nothing in the cloud was touched*. Proofs: the route test (the running one is stopped first, the stopped one is not "stopped" again, an unknown name is the job's error, nothing is exported or pushed); on the real page both cases end to end — the only copy with the typed name (a near-miss keeps the button disabled, the exact name enables it, the row is gone), and a copy in the cloud made by pushing through the real push form (one click, the row stays as remote) — red with the typed-name guard neutered (the button enabled without the name); and in the flow the pushed session is removed through the page before the pull, with the control that the stored object is byte for byte what it was (read back from the bucket by the acceptance's own signer in s3 mode). Counts: local 75, s3 79 (provisional; R2 not rerun). | Claude Code |
