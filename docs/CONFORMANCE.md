...
| **F-63** | med | harness | **Intermittent M8 restart failure, unresolved.** In one full-suite run `m8_session_state_lives_on_the_volume_and_vanishes_when_unmounted` failed at restart: `open .../mounts/m8-*/.nemr-state/projects: no such file or directory`. Observed **1 red in 12 full-suite runs**; the same test passes 12/12 in isolation. Cause not established — the plausible hypothesis is a race between unmount/remount and the container-init bind under parallel test load, but that is a hypothesis, not a diagnosis. | **Open.** Recorded rather than dismissed: an intermittent that only appears under parallel load is exactly the failure that gets re-classified as "flaky" and then ships. Not fixed, and the suite is not green-by-retry — the 11 green runs are reported alongside the 1 red, not instead of it. |
