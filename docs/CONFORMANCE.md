...
| **F-63** | **high** | VOL/PROC | **`nemr create` can report success having built a container that will never start.** `create_container` prepares the rootfs snapshot (`containers.rs:266`) and only afterwards writes the container record that references it (`containers.rs:268`). Between those two gRPC calls the snapshot has **no lease and no referrer**, which is containerd's definition of garbage. If the collector runs in that window the snapshot is deleted — and `create_container_record` still succeeds, because containerd does not validate that `snapshot_key` resolves. The failure surfaces later, at `nemr start`, as `snapshot "nemr-<project>" does not exist: not found`. The wrapper uses **no leases anywhere**, which is precisely what containerd's own client wraps this sequence in. | **Open — diagnosed, not fixed** (held for review per instruction). Proven by `cargo run -p nemr-containerd --bin gc_probe`: an unreferenced snapshot is present after `prepare`, absent after mutation churn past containerd 2.2.1's default `mutation_threshold = 100`. Reproduced in the suite's **supported serial mode** (1 red in 9 runs under CPU load), so it is not a parallelism artifact. Rates: parallel unloaded 1/31; parallel loaded 9/38; serial loaded 1/9. Spec consequence proposed: container creation must hold a containerd lease across snapshot preparation and record creation. |
