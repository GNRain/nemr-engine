...
| **F-63a** | med | harness | **A second failure signature under the same conditions, not yet diagnosed.** `m8` also fails at *restart* with `open .../mounts/<project>/.nemr-state/projects: no such file or directory` — the volume's state directory missing after a remount. Observed 3× in 38 loaded parallel runs and **0× in 9 serial runs**, so unlike F-63 it may well be a parallelism artifact, but that is not established. Separate mechanism from F-63: that one is a containerd snapshot, this one is a host directory. | **Open, undiagnosed.** Recorded separately rather than folded into F-63, because a single ID covering two mechanisms is how one of them gets closed by the other's fix. `volume_state_report` now captures mount state at the moment of failure, so the next occurrence will say whether the volume was mounted. |
