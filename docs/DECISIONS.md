...
1. *Command shape.* `nemr adopt <dir>` — the user names the directory; no disk scan, no discovery by heuristic (out of scope). It provisions a session like `create`, copies the tree in, copies the rewritten history in, and is thereafter an ordinary session that `push` exports.
