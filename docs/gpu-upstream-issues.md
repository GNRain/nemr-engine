...
output where we have it, **no speculation about cause**. Every identifier
below is from the run; the attachments each report ends with (full response
bodies, the server log) are in the box's terminal history, not here.
...
- Clients: Claude Code `2.1.240` via `ANTHROPIC_BASE_URL`
  (Anthropic-compatible `/v1/messages`); OpenAI Codex CLI (version not
  recorded) via `codex exec --oss --local-provider ollama`
  (`/v1/chat/completions`)
...
the agent's full tool definitions (Write, Read, Bash and the rest of its
set); the request body was not captured, so the reproduction below uses the
client itself.
...
| 5 | `qwen2.5-coder:7b` | Codex `--oss`, `-c sandbox_mode=workspace-write` | `/v1/chat/completions` | text, same as 4 — the sandbox was not the blocker |
...
end-of-turn value; no file was created; the directory stayed empty. Codex
with `qwen3-coder:30b` was not tried.
...
- **The model:** three models, 7B dense to 30B MoE, one result (the 30B
...
3. In an empty directory, with Claude Code installed:
   ...
   ```
4. Optionally the Codex path: `codex exec --oss --local-provider ollama -m
   qwen2.5-coder:7b --skip-git-repo-check <<< '<same prompt>'`, with or
   without `-c sandbox_mode=workspace-write`.

Attach when filing: one full JSON result from step 3 per model (the tool
call is in the text of `result`).
...
- Image `ghcr.io/ggml-org/llama.cpp:server-cuda` at
  `sha256:7f87a3bbe3143cdb857f5c84f82d2c15528be70ca0e7c5ade9a47d56b791f93f`;
  `llama-server --version` → `0.3.0-dev (build 10795, commit 6703d7894)`,
  built with GNU 14.2.0
- Host: WSL2 (Ubuntu), NVIDIA RTX 3070 8 GB; CUDA backend
- Model: `bartowski/Qwen2.5-Coder-7B-Instruct-GGUF`, `Q4_K_M`, downloaded
  by the server from Hugging Face; the same result with a Q4_K_M GGUF of the
  same model from another source (Ollama's library blob for
  `qwen2.5-coder:7b`, 4.68 GB, loaded with `-m`)
- Server flags: `--jinja -ngl 99 -c 8192 --host 0.0.0.0 --port 8080`
- Startup log confirms the template and residency:
  ```
  tokenizer.chat_template = {%- if tools %}…    (the model's own template, tool branch present)
  offloaded 29/29 layers to GPU
  CUDA0 model buffer size = 4168 MiB
  ```
...
OpenAI-compatible endpoint, the same schema in `tools[].function` form:
...
Run twice: once at 0.16 tok/s during a collision with a second server on the
card, once isolated at 62.9 tok/s. Same output both times.
...
Attach when filing: one full response body per endpoint, and the server log
from startup through the first request.
